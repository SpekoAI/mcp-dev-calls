...
/**
 * We authenticated the user fine, but the org-key endpoint would not hand us the
 * key — as of 2026-07 `/v1/api-keys/organization-credentials` requires a
 * "first-party dashboard session" and rejects the CLI's OAuth bearer with 403.
 * So the automatic no-paste fetch is unavailable; the wizard falls back to a
 * one-time key paste (the user IS signed in — this is not an auth failure, and
 * NOT "no organization"). If the platform later accepts the OAuth token again,
 * the auto-fetch just starts working with no client change.
 */
export class OrgKeyUnavailableError extends Error {
  readonly status: number;
  constructor(status: number, detail = "") {
    super(`automatic key fetch unavailable (HTTP ${status})${detail ? `: ${detail.slice(0, 120)}` : ""}`);
 ...
 * Throws OrgKeyUnavailableError on any non-2xx (the caller signs in fine but
 * can't auto-fetch the key — currently a 403 "first-party dashboard session"
 * requirement) so the wizard can fall back to a clean one-time paste.
 */
export async function fetchOrgKey(bearer: string): Promise<string> {
  ...
  if (!r.ok) {
    ...
    throw new OrgKeyUnavailableError(r.status, body);
  }
  const j = (await r.json()) as { mcpApiKey?: { key?: string } };
  const key = j.mcpApiKey?.key;
  if (!key) throw new OrgKeyUnavailableError(200, "response had no mcpApiKey.key");
  return key;
}
...
export async function browserLogin(log: (msg: string) => void = () => {}): Promise<string> {
  ...
  try {
    ...
    // Signed in. Try the no-paste auto-fetch; if the platform won't mint the key
    // for this token (OrgKeyUnavailableError), the caller falls back to a paste.
