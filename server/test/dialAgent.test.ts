 ...
 * and repaired, not trusted from cache.
 */
...
/**
 * Stateful fake that mirrors the platform behaviors under test: create AUTO-PICKS a
 * voice when none is supplied (create cannot express voice:null) and auto-attaches
 * the search_knowledge_base builtin tool; update applies voice/endCall verbatim.
 * Tests mutate `rows`/`tools` directly to simulate dashboard edits and the platform
 * KB backfill between dials, and reassign client methods to inject failures.
 */
function fakePlatform(seed: { rows?: FakeRow[]; tools?: FakeTool[] } = {}): FakePlatform {
