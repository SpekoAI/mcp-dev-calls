 ...
 * (reservation, order, availability, message, ...), not restaurant-specific. Warm + casual. The
 * objective is sanitized first so a smuggled behavior directive is never read aloud (B1).
 */
export function buildFirstMessage(callerName: string, objective: string): string {
  const purpose = sanitizeSpoken(objective).replace(/[.!?]+\s*$/, "").trim();
